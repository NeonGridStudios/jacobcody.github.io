# MindCachèAI  
Private. Secure. Effortless Voice Journaling.

<img width="300" height="400" alt="IMG_4099 2" src="https://github.com/user-attachments/assets/e7425d61-cd13-4338-a4c7-c944d05e9d8e" />

MindCachèAI is a fully on-device voice journal that captures your thoughts instantly, transcribes them using local processing, and organizes them into clean summaries — all without sending your data anywhere. Nothing leaves your phone unless *you* choose to share it.

<img width="300" height="400" alt="image3" src="https://github.com/user-attachments/assets/9991f3ff-250e-43c8-b441-5c5d66fbae94" />

## Key Features
- **100% On-Device Recording & Transcription**  
  No servers, no cloud, no accounts. Your audio and transcripts live only on your device.

- **Instant Summaries**  
  After speaking, MindCachèAI generates a concise summary for quick recall.

- **Editable Transcripts & Notes**  
  Clean up, correct, or expand your entries at any time.

- **Secure Access with Face ID / Touch ID**  
  Your journal is locked behind biometric authentication.

- **Audio Playback**  
  Revisit your recordings with enhanced clarity and smooth playback controls.

- **Simple, Calming Interface**  
  Minimalist design inspired by Apple’s aesthetic standard — fast, fluid, and distraction-free.

## Why It’s Different
MindCachèAI does *not* track, collect, or upload any personal information.  
Every recording, transcript, and note is stored locally and remains yours — permanently.

## Learn More
For full details, visit the Privacy Policy:
[Privacy Policy](PRIVACY_POLICY.md)
